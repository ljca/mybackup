...
##### singer_type #####
create table if not exists singer_type
(
...
##### singer_location #####
create table if not exists singer_location
(
...
-- song_id integer not null comment '',
  ...
  constraint fk_singer_type foreign key(singer_type_id) references singer_type(singer_type_id),
  -- constraint fk_
  constraint fk_location_singer foreign key (singer_location_id) references singer_location(location_id)
) AUTO_INCREMENT = 1;
...
###### song_path ######
create table if not exists song_path
(
  song_path_id integer auto_increment not null comment '',
...
###### lrc_info #######
create table if not exists lrc_info
(
  lrc_id integer auto_increment not null comment '',
  lrc_name varchar(255) comment '',
  primary key(lrc_id)
) AUTO_INCREMENT = 1;
...
(
  ...
  primary key(song_id),
  constraint fk_song_singer foreign key(singer_id) references singer_info(singer_id),
  constraint fk_song_type foreign key(song_type_id) references song_type(song_type_id),
  constraint fk_song_path foreign key(song_path_id) references song_path(song_path_id),
  constraint fk_song_lrc foreign key(lrc_id) references lrc_info(lrc_id)
