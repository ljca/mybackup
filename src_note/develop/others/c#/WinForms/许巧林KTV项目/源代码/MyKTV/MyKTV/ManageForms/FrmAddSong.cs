    ...
    public partial class FrmAddSong : Form
    {
        ...
        public string SelectFile()
        {
            ...
            if (ofdBrowse.ShowDialog() == DialogResult.OK)
            {
                ...
                Song.SongURL = ofdBrowse.FileName;
        ...
        public bool SearchSinger()
        {
            ...
            //if()
            //{
            DBHelper.sql = string.Format("select singer_name from [singer_info]  where singer_name like '%{0}%' ",txtSinger.Text);
            DBHelper.OpenCon();
            DBHelper.cm = DBHelper.GetCmd(DBHelper.sql);
            DBHelper.dreader = DBHelper.cm.ExecuteReader();
            while(DBHelper.dreader.Read())
            {
                string sa = DBHelper.dreader["singer_name"].ToString();
                sq += string.Format("{0}\t", sa);
            }
            MessageBox.Show(sq,"提示");
        ...
        public int InsertSong()
        {
            ...
            {
                DBHelper.sql = string.Format(@"insert into [song_info](song_name,song_ab,songtype_id,song_word_count,song_url) values('{0}','{1}',{2},{3},'{4}' ) ",txtSongN.Text,txtSongab.Text.Trim(),cboSongType.SelectedValue,txtSongN.Text.Length,Song.SongURL);//txtSinger.Text);go 
//insert into [singer_info](singer_name) values('{3}') go 
                try
                {
                    DBHelper.OpenCon();
                    DBHelper.cm = DBHelper.GetCmd(DBHelper.sql);
                    int s = DBHelper.cm.ExecuteNonQuery();
                    if (s != 0)
                    {
